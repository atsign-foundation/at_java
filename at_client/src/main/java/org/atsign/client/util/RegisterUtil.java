...
public class RegisterUtil {
   ...
   */
  public String getFreeAtsign(String registrarUrl, String apiKey) throws AtException, IOException {
   ...
   *
   * @param registrarUrl - URL of the atsign registrar API
   * @param apiKey - Super API belonging to the user that has suitable access to the registrar API
   * @return Map containing atSign and ActivationKey as separate entries. Ex: {"atSign":
   *         "randomAtsign", "ActivationKey": "valueActivationKey"}
   ...
   */
  public Map<String, String> getAtsignWithSuperApiKey(String registrarUrl, String apiKey)
   ...
   * @param registrarUrl - URL of the atsign registrar API
   * @param apiKey - Super API belonging to the user that has suitable access to the registrar API
   * @param atsign - [Optional] User can choose to provide atsign. Note: The final atsign will be in
   *        the following format: SuperApi prefix + atsign + SuperApi postfix
   * @param activationKey - [Optional] User can choose to provide an ActivationKey which will be set
   *        as activationKey for the corresponding atsign.
   * @return Map containing atSign and ActivationKey as separate entries. Ex: {"atSign":
   *         "randomAtsign", "ActivationKey": "valueActivationKey"}
   * @throws AtException - thrown if invalid method parameters provided
   * @throws IOException - thrown if anything goes wrong while using the HttpsURLConnection.
  ...
  /**
   * Accepts email and an unpaired atsign. Method pairs free atsign with email. Sends the
   * one-time-password to the provided email. Returns bool, true if OTP sent or False otherwise.
   *
   * @param email - email initially provided to register this email
   * @param atsign - atsign that is being registered
   * @param registrarUrl - URL of the atsign registrar API
   * @param apiKey - API key to authenticate connection to atsign registrar API
  ...
  /**
   * Accepts email, unpaired atsign, and the otp received on the provided email. Validates the OTP
   * against the atsign and registers it to the provided email if OTP is valid. Returns the CRAM
   * secret of the atsign which is registered.
   *
   * @param email - email initially provided to register this email
   * @param atsign - atsign that is being registered
   * @param otp - one-time-password received on provided email for provided atsign
   * @param registrarUrl - URL of the atsign registrar API
   * @param apiKey - API key to authenticate connection to atsign registrar API
   * @param confirmation - Mandatory parameter for validateOTP API call. First request to be sent with
   *        confirmation as false, in this case API will return cram key if the user is new otherwise
   ...
   * @throws IOException thrown if anything goes wrong while using the HttpsURLConnection.
   * @throws AtException Case 1: If user has exhausted 10 free atsign quota Case 2: If API response is
   *         anything other than HTTP_OK/Status_200.
   */
    ...
    if (httpsConnection.getResponseCode() == HttpsURLConnection.HTTP_OK) {
      ...
      if (responseDataStringObject.containsKey("message")
        ...
        return "retry";
          ...
          && responseDataStringObject.get("message").contains("You already have the maximum number of free @signs")) {
  ...
  /**
   * Activates the atsign provided using activationKey provided through method parameters
   *
   * @param registrarUrl - URL of the atsign registrar API
   * @param apiKey - Super API belonging to the user that has suitable access to the registrar API
   * @param atsign - atsign received as a response from GetAtsignV3
   * @param activationKey - activationKey corresponding to the atsign being provided
