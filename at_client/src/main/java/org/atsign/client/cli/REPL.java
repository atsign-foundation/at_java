...
public class REPL {
      ...
      switch (eventType) {
        case decryptedUpdateNotification:
          ...
          System.out.println("  => Notification ==> \033[31m Key: [" + sharedKey + "]  ==> EncryptedValue [" + value
              + "]  ==> DecryptedValue [" + decryptedValue + "]" + "\033[0m");
          writePrompt();
          break;
  ...
  public void printHelpInstructions() {
    ...
    System.out.println(ansi()
        .render("    1) By default, REPL treats input as atProtocol commands. Use / for additional commands listed below"));
    System.out.println(ansi()
        .render("    2) In the usage examples below, it is assumed that the atSign being used is @|bold,green @alice|@"));
    System.out.println();
    System.out.println(ansi().render("  @|bold,magenta help|@ or @|bold,magenta /help|@ - print this help message"));
    System.out.println();
    System.out.println(ansi()
        .render("  @|bold,magenta /scan|@ @|bold,green [regex]|@ - scan for all records, or all records whose keyNames match the regex (e.g. _scan test@alice.*)"));
    System.out.println();
    System.out.println(ansi()
        .render("  @|bold,magenta /put|@ @|bold,green <atKeyName>|@ @|bold,blue <value>|@ - create or update a record with the given atKeyName and with the supplied value - for example:"));
    System.out.println(ansi()
        .render("    @|bold,magenta /put|@ @|bold,green test@alice|@ @|bold,blue secret secrets|@ will create or update a 'self' record (a record private just to @alice)"));
    System.out.println(ansi()
        .render("    @|bold,magenta /put|@ @|bold,green @bob:test@alice|@ @|bold,blue Hello, Bob!|@ will create or update a record encrypted for, and then shared with, @bob"));
    System.out.println();
    System.out.println(ansi()
        .render("  @|bold,magenta /get|@ @|bold,green <atKeyName>|@ - retrieve a value from the record with this atKeyName - for example:"));
    System.out.println(ansi()
        .render("    @|bold,magenta /get|@ @|bold,green <atKeyName>|@ - retrieve a value from the record with this atKeyName (e.g. _get test@alice)"));
    System.out.println();
    System.out.println(ansi()
        .render("  @|bold,magenta /delete|@ @|bold,green <atKeyName>|@ - delete the record with this atKeyName (e.g. _delete test@alice)"));
    System.out.println();
    System.out.println(ansi()
        .render("  @|bold,red NOTE:|@ @|bold,magenta put, get|@ and @|bold,magenta delete|@ will append the current atSign to the atKeyName if not supplied"));
    System.out.println();
  }
}
