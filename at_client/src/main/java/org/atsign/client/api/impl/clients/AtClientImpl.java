...
public class AtClientImpl implements AtClient {
		...
		case updateNotification: {
			...
			if (eventData.get("value") != null) {
				...
				try {
					...
					String decryptedValue = EncryptionUtil.aesDecryptFromBase64(encryptedValue,
							encryptionKeySharedByOther);
	 ...
	 * @param throwExceptionOnErrorResponse sometimes we want to inspect an error
	 *                                      response, sometimes we want to just
	 *                                      throw an exception
	 * @return a Secondary Response
	 * @throws AtException if the response from the Secondary starts with 'error:',
...
// ============================================================================================================================================
	...
	private String _getSharedByMeWithOther(SharedKey sharedKey) throws AtException {
		...
		// fetch local - e.g. if I'm @bob, I would first
		// "llookup:@alice:some.key.name@bob"
		...
		try {
			...
			throw new AtException("Failed to " + "decrypt value with shared encryption key" + " : " + e.getMessage(),
					e);
		}
	}
	...
	private String _getSharedByOtherWithMe(SharedKey sharedKey) throws AtException {
		...
		// first, try to fetch cached - e.g. if I'm @bob, I would first
		// "llookup:cached:@bob:some.key.name@alice"
		...
		if (rawResponse.isError) {
			if (rawResponse.error.contains("AT0015-key not found")) {
				// next, try to fetch from remote - e.g. if I'm @bob,
				// "lookup:some.key.name@alice" - note that "@bob" is NOT required
...
//            response = secondary.executeCommand(command, true);
	...
	private String _put(PublicKey publicKey, String value) {
		throw new RuntimeException("Not Implemented");
	}
	...
	public CompletableFuture<String> getRawKeys(String regex){
		return CompletableFuture.supplyAsync(() -> {
			try {
				Secondary.Response rawResponse;
				String command = "scan";
	...
	//
	// Internal utility methods. Will move these to another class later, so that
	// other AtClient implementations can easily use them.
	//
	private String getEncryptionKeySharedByMe(SharedKey key) throws AtException {
		...
		// When we stored it, we encrypted it with our encryption public key; so we need
		// to decrypt it now with our encryption private key
		try {
	...
	private String getEncryptionKeySharedByOther(SharedKey sharedKey) throws AtException {
		...
		String what = "";
		try {
			// First, try to fetch cached - e.g. if I'm @bob,
			// llookup:cached:@bob:shared_key@alice
			...
			String sharedSharedKeyDecryptedValue = EncryptionUtil.rsaDecryptFromBase64(rawResponse.data,
					keys.get(KeysUtil.encryptionPrivateKeyName));
	...
	private String createSharedEncryptionKey(SharedKey sharedKey) throws AtException {
		...
		if (theirPublicEncryptionKey == null) {
			throw new AtException(" public key " + sharedKey.sharedWith
		...
		try {
			...
			throw new AtException(
					"Failed to generate AES key for sharing with " + sharedKey.sharedWith + " : " + e.getMessage(), e);
		}

		String what = "";
		try {
			// Encrypt key with the other at-sign's publickey and save it
			...
			String encryptedForUs = EncryptionUtil.rsaEncryptToBase64(aesKey,
					keys.get(KeysUtil.encryptionPublicKeyName));

			what = "save encrypted shared key for us";
			secondary.executeCommand("update:" + "shared_key." + sharedKey.sharedWith.withoutPrefix()
					+ sharedKey.sharedBy + " " + encryptedForUs, true);

			what = "save encrypted shared key for them";
			int ttr = 24 * 60 * 60 * 1000;
			secondary.executeCommand("update:ttr:" + ttr + ":" + sharedKey.sharedWith + ":shared_key"
					+ sharedKey.sharedBy + " " + encryptedForOther, true);
		} catch (Exception e) {
			throw new AtException("Failed to " + what + " : " + e.getMessage(), e);
		}
	...
	@Override
	public CompletableFuture<NotificationResult> notifyText(String text, AtSign other) {
		return CompletableFuture.supplyAsync(() -> {
			try {
				return new NotificationImpl(secondary).notifyText(text, this.atSign, other);
			} catch (Exception e) {
				throw new CompletionException(e);
			}
		});
